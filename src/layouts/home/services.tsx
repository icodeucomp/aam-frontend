    ...
    <Background src="/images/construction-progress.webp" className="grid justify-center grid-cols-2 mx-auto md:grid-cols-3 lg:grid-cols-4 max-w-screen-2xl filter-image">
      {serviceLists.map((item, index) => {
        return (
          <div
            key={index}
            className="px-8 pt-24 space-y-8 text-center duration-300 border-2 md:pt-32 hover:pt-16 min-h-400 hover:bg-primary"
          ...
          >
            ...
            />
            <h3 className={`font-semibold ${imgHover === index ? "text-lg sm:text-xl xl:text-2xl" : "text-xl sm:text-2xl xl:text-3xl"}`}>{item.title}</h3>

            <p className={`text-xs lg:text-sm transition-all duration-300 ${imgHover === index ? "h-auto opacity-100" : "h-0 opacity-0 "}`}>{item.description}</p>
          </div>
        );
      })}
    </Background>
