...
export const CardCertification = ({
  ...
  return (
    <div
      className={`card-certification group ${lastIndex ? "rounded-b-lg" : "rounded-none"} ${firstIndex ? "rounded-t-lg" : "rounded-none"} ${
        selected === slug && "bg-primary"
      }`}
      onClick={() => setSelected(slug)}
    >
      <div className="flex items-center gap-4">
        <Img src="/temp-image.png" alt="temporary" className="aspect-square w-24" />
        <div className="space-y-2">
          <h5 className={`text-sm sm:text-base md:text-lg font-semibold line-clamp-2 ${selected === slug ? "text-light" : "text-primary"}`}>
            {name}
          </h5>
