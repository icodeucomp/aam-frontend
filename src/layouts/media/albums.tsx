...
export const Albums = () => {
  ...
  const { response: albums, loading } = useGet<ResponseAlbumsTypes>({
    path: "/albums",
            ...
            <>
              {albums?.data.map((item, index) => (
                <Motion
                  ...
                  delay={index * 0.1}
                  key={index}
                  className="relative grid grid-cols-2 gap-4 p-4 duration-300 rounded-lg border border-secondary/40 shadow-lg min-h-300 bg-light hover:bg-secondary group"
                >
                  <Img src={item?.header || "/temp-business.webp"} alt={item.name} className="w-full h-[268px] rounded-lg" cover />
                  <div className="flex items-center text-center text-secondary group-hover:text-light line-clamp-2">
                    <h4 className="text-lg font-semibold sm:text-xl">{item.name}</h4>
                  </div>
                  <span onClick={() => setOpenModalIndex(item.slug)} className="absolute flex items-center gap-1 text-sm cursor-pointer bottom-4 right-4 text-secondary group-hover:text-light">
