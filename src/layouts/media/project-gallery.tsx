...
export const ProjectGallery = () => {
  ...
  const { response: projects, loading } = useGet<ResponseBusinessesSectorTypes>({
    path: "/projects",
      ...
      {loading ? (
        ...
        <>
          {projects?.data && projects?.data.length < 1 ? (
            <h3 className="w-full py-16 text-lg font-semibold text-center min-h-400 sm:text-2xl md:text-3xl text-gray/50">The projects is not found</h3>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:grid-rows-2">
                  ...
                  <div key={index} className={`cursor-pointer ${grid}`} onClick={() => setOpenModalIndex(item.slug)}>
                    <Motion tag="div" initialY={50} animateY={0} duration={0.5} delay={index * 0.1}>
                      <Background
                        ...
                        className={`items-end w-full p-4 sm:p-6 filter-image ${index === 0 || index === 1 ? "min-h-400" : "min-h-400 lg:min-h-48"}`}
                        isHover
                      >
