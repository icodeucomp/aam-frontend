  ...
  return (
    <>
      <div className="flex flex-col items-center justify-between gap-4 px-2 pb-2 border-b-2 border-secondary sm:items-end sm:flex-row">
        <span className="flex items-center gap-2 text-dark-blue h-12">
        ...
        {title !== "" && content !== "" && (
          <Button onClick={handleSubmitForm} className="flex items-center gap-2 btn-secondary">
      ...
      {loading ? (
        ...
        <div className="relative w-full max-w-screen-sm px-4 py-10 mx-auto space-y-4 sm:px-8">
          <div className="relative w-full">
            <input type="text" id="title" value={title} className="floating-input peer" placeholder=" " required onChange={(e) => setTitle(e.target.value)} />
            <label
              htmlFor="title"
          ...
          {error && !title && <small className="text-red-500">Please input title!</small>}
