          ...
          <article key={index} className="w-full max-w-xs p-4 space-y-4 duration-300 rounded-md card-shadow text-dark-blue bg-light">
            ...
            <div className="flex items-center justify-center gap-4">
              <Link href={`/admin/dashboard/article/show/${item.slug}`}>
                <button className="p-2 duration-300 border rounded-full border-secondary bg-light hover:bg-secondary group">
                  <BiShowAlt size={20} className="text-secondary group-hover:text-light" />
                </button>
              </Link>
              <Link href={`/admin/dashboard/article/edit/${item.slug}`}>
                <Button className="w-full btn-secondary">Edit</Button>
              </Link>
...
export const Articles = () => {
  ...
  // use debounce for search input to minimize API calls
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-between gap-4 my-4 sm:flex-row">
        ...
        <Link href="/admin/dashboard/article/add">
          <Button className="btn-secondary">Add New Blog</Button>
        </Link>
      </div>
      {loading ? (
      ...
      <div className="pt-8">
        <Pagination page={page} totalPage={totalPage} setPage={setPage} isNumber color="secondary" />
      </div>
    </>
  );
};
