            ...
            <form onSubmit={handleSubmitForm} className="w-full mt-4 space-y-4">
              <div className="relative text-center">
                ...
                {project.header && project.header.url !== "" ? (
                  <Img src={project.header?.url || "/temp-business.webp"} alt={project.header.slug} className="mx-auto rounded-lg w-60 aspect-square" cover />
                ) : (
                  <>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 sm:grid-cols-3">
                  {project?.media?.map((image, index) => (
                    <div key={index} className="relative">
                      <button onClick={() => handleDeleteImage(index)} className="absolute flex items-center justify-center w-5 h-5 rounded-full -top-2 -right-2 z-1 bg-red-500">
