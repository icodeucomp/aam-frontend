...
export const Show = ({ slug }: { slug: string }) => {
  ...
  // fetch api to show data business by slug
  const { response: business, loading } = useGet<ResponseBusinessTypes>({ path: `/business/${slug}` });

  return (
    <div ref={ref} className="absolute top-2.5 right-1.5">
      <button onClick={toggleModal} className="p-1.5 duration-300 border rounded-full border-secondary bg-light hover:bg-secondary group">
          ...
          <Modal isVisible={modal} onClose={toggleModal} className="max-w-2xl">
            {loading ? (
              ...
              <div className="space-y-4 text-dark-blue">
                ...
                <Img src={business?.data.header.url || "/temp-business.webp"} alt={business?.data.title || ""} className="mx-auto rounded-lg w-80 aspect-square" cover />
