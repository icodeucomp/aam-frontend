...
export const EditProduct = ({ slugProduct, businessId, slug }: { slugProduct: string; businessId: number; slug: string }) => {
  ...
  const { response: resProduct } = useGet<ResponseBusinessSectorTypes>({ path: `/products/${slugProduct}` });
      ...
      <AnimatePresence>
        {modal && (
          <Modal isVisible={modal} onClose={handleClose} className="max-w-2xl">
            <h3 className="text-lg font-semibold text-center sm:text-start text-primary">Edit Product</h3>
            <form onSubmit={handleSubmitForm} className="w-full mt-4 space-y-4">
              <div className="relative w-full">
                <input type="text" id="title" className="floating-input peer" placeholder=" " value={product.title} onChange={(e) => setProduct((prev) => ({ ...prev, title: e.target.value }))} />
                <label
                  htmlFor="title"
              ...
              {errors && !product.title && <small className="w-full text-red-500">Enter title please!</small>}
              <div className="relative w-full">
                <input
                  type="text"
                  id="description"
              ...
              {errors && !product.description && <small className="w-full text-red-500">Enter description please!</small>}
              <div className="space-y-4">
                ...
                {errors && !product.media?.length && <small className="w-full text-red-500">Enter images please!</small>}
                <div className="grid grid-cols-2 gap-4 mt-4 sm:grid-cols-3">
                  {product?.media?.map((image, index) => (
                    <div key={index} className="relative">
                      <button onClick={() => handleDeleteImage(index)} type="button" className="absolute flex items-center justify-center w-5 h-5 rounded-full -top-2 -right-2 z-1 bg-red-500">
