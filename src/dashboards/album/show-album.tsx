...
export const ShowAlbum = ({ slug }: { slug: string }) => {
  ...
  // fetch api to show data album by slug
  const { response: album, loading } = useGet<ResponseAlbumTypes>({ path: `/albums/${slug}` });
            ...
            {loading ? (
              ...
              <div className="space-y-4 md:min-w-xl">
                ...
                <div className="relative text-center">
                  <Img src={album?.data.header || "/temp-business.webp"} alt={album?.data.name || slug} className="w-64 mx-auto rounded-lg aspect-video" cover />
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-4">
                  {album?.data.medias?.map((image, index) => (
                    <div key={index} className="relative">
                      <Img src={image.url || "/temp-business.webp"} alt={`Selected image ${index + 1}`} className="w-40 rounded-lg aspect-video" cover />
                    </div>
                  ))}
                </div>
              </div>
